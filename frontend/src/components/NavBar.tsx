    ...
    <>
      <nav className="border-border bg-surface-card/95 sticky top-0 z-40 border-b backdrop-blur">
        <div className="max-w-container mx-auto flex items-center justify-between px-4 py-2">
          ...
          {/* Right side: help + FY selector + badge + logout */}
          <div className="flex items-center gap-2">
            <button
              onClick={() => setHelpOpen(true)}
              className="text-text-secondary hover:bg-surface-muted hover:text-text border-current/20 flex h-7 w-7 items-center justify-center rounded-full border text-xs font-bold transition-colors"
              aria-label="Help"
            >
              ?
            </button>
